...
// Здесь могут быть глобальные операции завершения
    ...
    @Test
    @DisplayName("Проверка корректной скидки")
    void testcalculateDiscount(){
        assertEquals(5000, calculator.calculateDiscount(10000,50));
    }

    @Test
    @DisplayName("Проверка диапазона суммы")
    void testCorrectSumm() {
        assertThrows(ArithmeticException.class, () -> calculator.calculateDiscount(-1,50));
    }

    @Test
    @DisplayName("Проверка нижнего диапазона скидки")
    void testCorrectDiscountDown() {
        assertThrows(ArithmeticException.class, () -> calculator.calculateDiscount(10000,-1), "Скидка ниже 0");
    }

    @Test
    @DisplayName("Проверка верхнего диапазона скидки")
    void testCorrectDiscountUp() {
        assertThrows(ArithmeticException.class, () -> calculator.calculateDiscount(10000,101), "Скидка выше 100");
    }
}